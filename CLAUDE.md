...
- Released as `v1.0.0` (current). v0.8.0 shipped the token-economy subsystem (daemon + tracker + telemetry + hooks + `read`/`gain`/`plugin`/`config` commands); v1.0.0 is the first stable release and ships the public marketplace-based plugin flow (`/plugin marketplace add browzeremb/skills`) — the old file-drop `browzer plugin install` is now a printer of marketplace instructions.
...
| `POST /api/ask` | `ask` — 3-tier `workspaceId` fallback, never sends empty | v0.6.0 |
