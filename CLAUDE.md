...
2. **Workflow read + persist (legacy `workflow.json` features only)** — the surviving workflow surface is intentionally narrow: **reads** via `browzer get-step <ID> --id <feat>` (top-level, also exposed under `browzer workflow get-step`) which by default emits a rich markdown view assembled from embedded `view/templates/*.md.tmpl` (one per phase) and `--json` for the raw `#StepView`; **persists** via `browzer save-step <PHASE> --id <feat> [--from <file>] [--stdin] [--hint-fixes]` which validates the payload against the CUE schema and writes atomically (tmp + rename + optional fsync). `save-step` rejects `--async` (exit 1 with `unknown flag: --async`); use `--await` (default) or `--sync`. `--hint-fixes` makes enum / unknown-field violations emit a worked example (`expected one of [...]; example: "<key>": "<value>"`) — opt-in, off by default. A sibling **`save-step-batch --from PHASE=PATH ...`** persists multiple phases atomically under a single advisory flock + single CUE pass against the in-memory mutated document; failure rolls back without writing. A small set of structured mutators remains for batch / bookkeeping use cases — `workflow {append-step,append-steps,backfill-elapsed,validate,describe-step-type,schema,init}` — each acquiring an advisory flock and CUE-validating before commit. Every other historical mutator verb (`patch`, `query`, `set-config`, `set-status`, `set-current-step`, `complete-step`, `update-step`, `audit-model-override`, `reapply-additional-context`, `truncation-audit`, `append-dispatch{,es}`, `append-agent`, `append-review-history`, `save`, `get-config`, `set-finding-status`) was deleted in CLI v3.0.0. In the **markdown-chains** pipeline (current), skill bodies write phase output as plain `.md` files into `docs/browzer/<feat>/staging/` and the next phase reads them with the `Read` tool; the `staging/` directory itself is preserved (gitignored, auto-created at orchestrator entry), only the historical `staging/ → autosave hook → save-step → workflow.json` chain was retired in v5.0.0. `get-step` / `save-step` / `save-step-batch` / `describe-step-type` therefore exist to service older features that still carry a `workflow.json`; do NOT introduce new call-sites in markdown-chains skills.
...
| `--async`          | daemon FIFO (generic mutators only; removed from save-step/save-step-batch) | <50ms — daemon completes durably in background |
| `--await`          | daemon + fsync wait        | ~50–120ms — durable when the call returns     |

Override via `BROWZER_WORKFLOW_MODE=sync|async|await`. CI/test paths force standalone (`--sync`) when a long-running daemon binary may be stale. For `save-step` and `save-step-batch`, the effective default is `--await` (the `--async` flag is rejected with exit 2).

> **`save-step` / `save-step-batch` exception**: `--async` was **removed** from both verbs (exits `2` with `--async is deprecated; remove the flag`). These verbs default to `--await` (daemon + fsync). Callers that previously passed `--async` must remove the flag; `--await` is the correct equivalent. Generic mutators still accept `--async` as the default fire-and-forget mode.
>
> **`--async` removal status**: `--async` has been removed from `save-step` and `save-step-batch` (already exits `2` when passed). For generic workflow mutators, `--async` is still accepted but deprecated — migrate to `--await` (durable, same latency class) or `--sync` (in-process). The environment variable `BROWZER_WORKFLOW_MODE=async` is also deprecated; set `BROWZER_WORKFLOW_MODE=await` or omit it to accept the per-verb default.

The daemon (`internal/daemon/`) speaks JSON-RPC over a Unix socket, hosts the read-filter pipeline (AST trimming for `browzer read --filter auto`), the manifest + session caches, and the `workflow_queue` for `--await` / generic-mutator `--async` writes (note: `save-step` and `save-step-batch` no longer use `--async`). Lifecycle: `browzer daemon start --background`, `browzer daemon status --json`, `browzer daemon stop`.
